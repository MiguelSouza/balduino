...
export default class UserRepository implements IUserRepository {    
    ...
    async update(user: User): Promise<void> {
        await this.connection?.query(
            ...
            WHERE user_id = $9`,
            [user.name, user.cpf, user.password, user.email, user.birthday, user.admin, user.active, new Date(), user.userId]
        );
    }

    async getById(userId: string): Promise<User> {
        const result = await this.connection?.query('SELECT * FROM balduino.user WHERE user_id = $1', [userId]);
    ...
    async delete(userId: string): Promise<void> {
        this.connection?.query('delete from balduino.user where user_id = $1', [userId]);
    }
}